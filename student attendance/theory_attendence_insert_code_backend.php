...
if(isset($_POST['checking_insert']))
{
    ...
    $date=$data[0][3];
    $sub_id=$data[0][2];
    $query1="SELECT * FROM `attend_theory_class` WHERE DA_TE='$date' AND SUB_theory_ID='$sub_id';";
    ...
    if ($num == 0)
    {
        foreach($data as $row) 
        {
            $query .= "INSERT INTO attend_theory_class(COLLEGE_ROLL_NUMBER,TEACHER_ID,SUB_THEORY_ID,DA_TE,DEPARTMENT_ID,YE_AR,SEMESTER,
